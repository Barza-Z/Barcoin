...
uint256 hashGenesisBlock("0x28ade983939d07a9fab07866a9c7ee84a4436f40ae31aafd53d5f03ab683cdf7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Barcoin: starting difficulty is 1 / 2^12
...
{
    // Barcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1680000); // Barcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Barcoin: 3.5 days
static const int64 nTargetSpacing = 0.5 * 60; // Barcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Barcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x551edd8c4653250469e195cdc2bed6deba6b190653f04ccfd0bb169fa5bd0846");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bar coin is here";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040a85dd62917234ad5d95a511cb242112cc68e79fab61c868bf350c2cda314ec4d4045e75c03d3a636c83b4ef1a1524dc44c082fe196ad7394bb513a53da495cf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1638172035;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088124444;

        if (fTestNet)
        {
            block.nTime    = 1638171979;
            block.nNonce   = 385783368;
        }
		/////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        assert(block.hashMerkleRoot == uint256("0x3cfac2b8d14a44bc93deeb55e895fdd86f6c3e6462e82ad75ec6376a42edf2dd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xcf, 0xb4, 0xd7 }; // Barcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BarcoinMiner(CWallet *pwallet)
{
    printf("BarcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("barcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BarcoinMiner, pwallet));
}
